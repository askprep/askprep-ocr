...
function tsvToRawContent(tsv, ratio) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (currentline[level] === "2") {
    ...
    } else if (currentline[level] === "3") {
      // If its a paragraph
      const len = content["blocks"].length;
      if (
        currentline[indexOfHeader["block_num"]] != "1" ||
        currentline[indexOfHeader["par_num"]] != "1"
      )
        content["blocks"][len - 1]["text"] = content["blocks"][len - 1][
          "text"
        ].concat("\n");
    } else if (currentline[level] === "4") {
      // If it's a line
      // const len = content["blocks"].length;
      // content["blocks"][len-1]["text"] = content["blocks"][len-1]["text"].concat("\n");
    } else if (currentline[level] === "5") {
      // If it is word
      const len = content["blocks"].length;
      ...
      // else
      content["blocks"][len - 1]["text"] = content["blocks"][len - 1]["text"]
        .concat(" ")
        .concat(currentline[indexOfHeader["text"]]);
    }
  }
  ...
  return new Promise(function(resolve, reject) {
    tesseract.process(filename, options, function(err, tsv) {
      if (err) {
        ...
        let ratio = 1;
        if (dimensions.width > max_width) ratio = dimensions.width / max_width;
